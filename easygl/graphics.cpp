...
void auto_zoom_in (float x, float y, bool zoomin){
    t_point old_cord = get_visible_world().get_center();
    
    float xchange = x - old_cord.x;
    float ychange = y - old_cord.y;
    
    trans_coord.ytop += ychange;
    trans_coord.ybot += ychange;
    trans_coord.xleft += xchange;
    trans_coord.xright += xchange;
    
    update_transform();
    
    if (zoomin)
        handle_zoom_in(x,y,NULL);
    else
        handle_zoom_out(x,y,NULL);
...
void draw_surface(const Surface& surface, float x, float y) {
    if (surface.getSurface() != NULL) {
        cairo_set_source_surface(x11_state.ctx, surface.getSurface(),
                ...
                (yworld_to_scrn(y) - cairo_image_surface_get_height(surface.getSurface())));
